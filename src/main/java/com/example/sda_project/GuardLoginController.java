...
public class GuardLoginController {
    ...
    private void onNextButtonClick(ActionEvent event) {
        ...
        if (name.isEmpty() || password.isEmpty()) {
            ...
            boolean isValidGuard = DataBaseHelper.checkGuardLogin(name, password);
            if (isValidGuard) {
                // Navigate to Admin Menu if credentials are valid
                navigateToGuardMenu(event);
            } else {
                showAlert(AlertType.ERROR, "Login Error", "Guard not found. Please check your credentials.");
            }
        }
    }
    ...
    private void navigateToGuardMenu(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("guard_menu.fxml"));
            Parent root = loader.load();
            int guard_id = 0;
            for(Guard g: prms.guards)
            {
                if (g.getName().equals(nameField.getText()) && g.getPassword().equals(passwordField.getText()))
                {
                    guard_id = g.getId();
                }
            }
            System.out.println(guard_id);
            GuardMenu controller = loader.getController();
            controller.setGuard_id(guard_id);
